const dimensionMeta = {
  ...
  "DRUNK": {
    ...
    const TYPE_IMAGES = {
      "IMSB": "assets/types/imsb.png",
      "BOSS": "assets/types/boss.png",
      "MUM": "assets/types/mum.png",
      "FAKE": "assets/types/fake.png",
      "Dior-s": "assets/types/dior-s.png",
      "DEAD": "assets/types/dead.png",
      "ZZZZ": "assets/types/zzzz.png",
      "GOGO": "assets/types/gogo.png",
      "FUCK": "assets/types/fuck.png",
      "CTRL": "assets/types/ctrl.png",
      "HHHH": "assets/types/hhhh.png",
      "SEXY": "assets/types/sexy.png",
      "OJBK": "assets/types/ojbk.png",
      "JOKE-R": "assets/types/joke-r.png",
      "POOR": "assets/types/poor.png",
      "OH-NO": "assets/types/oh-no.png",
      "MONK": "assets/types/monk.png",
      "SHIT": "assets/types/shit.png",
      "THAN-K": "assets/types/than-k.png",
      "MALO": "assets/types/malo.png",
      "ATM-er": "assets/types/atm-er.png",
      "THIN-K": "assets/types/thin-k.png",
      "SOLO": "assets/types/solo.png",
      "LOVE-R": "assets/types/love-r.png",
      "WOC!": "assets/types/woc.png",
      "DRUNK": "assets/types/drunk.png",
      "IMFW": "assets/types/imfw.png"
    };
  ...
  },
  {
    ...
    const SPECIAL_TYPE_CODES = ['HHHH', 'DRUNK'];
  ...
  "So3": {
    ...
    const SITE_URL = 'https://sbti.jerryz.com.cn';
    const SITE_HOST = 'sbti.jerryz.com.cn';
    const RESULT_STORAGE_KEY = 'sbti:last-result:v2';
    ...
    const screens = {
      get 'intro'() { return document.getElementById('intro'); },
      get 'test'() { return document.getElementById('test'); },
      get 'result'() { return document.getElementById('result'); }
    };
    ...
    const viewSavedResultBtn = document.getElementById('viewSavedResultBtn');
    const savedResultNote = document.getElementById('savedResultNote');
    ...
    function getQrUrl(size = 240, format = 'png') {
      return `https://quickchart.io/qr?text=${encodeURIComponent(SITE_URL)}&size=${size}&margin=1&dark=1e2a22&light=ffffff&format=${format}`;
    }

    async function ensureQrDataUrl(size = 280) {
      if (app.qrDataUrl) return app.qrDataUrl;
      const response = await fetch(getQrUrl(size, 'base64'));
      if (!response.ok) {
        throw new Error('QR request failed');
      }
      const base64 = (await response.text()).trim();
      app.qrDataUrl = base64.startsWith('data:')
        ? base64
    ...
    function renderShareQr() {
      ...
      shareQr.referrerPolicy = 'no-referrer';
      shareQr.src = getQrUrl(240, 'png');
      const shareUrlText = document.getElementById('shareUrlText');
      if (shareUrlText) shareUrlText.textContent = SITE_HOST;
    }
    ...
    function buildPersistedResult(result, imageSrc) {
      return {
        savedAt: Date.now(),
        imageSrc: imageSrc || '',
        result: {
    ...
    function saveResultToStorage(result, imageSrc) {
      if (!storageAvailable()) return;
      try {
        const payload = buildPersistedResult(result, imageSrc);
        localStorage.setItem(RESULT_STORAGE_KEY, JSON.stringify(payload));
        app.savedResult = payload;
        renderSavedResultCta();
      } catch (error) {
        console.error(error);
      }
    }
    ...
    function loadSavedResultFromStorage() {
      ...
      try {
        ...
        if (!parsed || !parsed.result || !parsed.result.finalType) return null;
        return parsed;
      } catch (error) {
        console.error(error);
        return null;
    ...
    function renderSavedResultCta() {
      const snapshot = app.savedResult;
      if (!snapshot || !viewSavedResultBtn || !savedResultNote) {
        if (viewSavedResultBtn) viewSavedResultBtn.hidden = true;
        if (savedResultNote) savedResultNote.hidden = true;
        return;
      }

      const type = snapshot.result.finalType;
      viewSavedResultBtn.hidden = false;
      savedResultNote.hidden = false;
      viewSavedResultBtn.textContent = '查看上次结果';
      savedResultNote.textContent = `已为你保存上次结果：${type.code}（${type.cn}） · ${formatSavedTime(snapshot.savedAt)}`;
    }
    ...
    function getResultShareText() {
      if (!app.currentResult) return getDefaultShareText();
      const { finalType, badge, sub } = app.currentResult;
      return `我测出的 SBTI 结果是 ${finalType.code}（${finalType.cn}）。${badge}。${sub}`;
    }
    ...
    async function shareResult() {
      await sharePayload({
        title: app.currentResult
          ? `我的 SBTI 结果：${app.currentResult.finalType.code}（${app.currentResult.finalType.cn}）`
          : 'SBTI 人格测试',
        text: getResultShareText(),
        url: SITE_URL
      });
    }
    ...
    function renderRadarChart(result) {
      ...
      if (app.radarChart) {
        app.radarChart.destroy();
        app.radarChart = null;
      }

      const labels = dimensionOrder.map(dim => dim);
      const fullLabels = dimensionOrder.map(dim => dimensionMeta[dim].name);
      const expandedLabels = fullLabels.map(label => {
        const [code, ...rest] = label.split(' ');
        return rest.length ? [code, rest.join(' ')] : [label];
      });
      const scores = dimensionOrder.map(dim => Number(result.rawScores[dim] || 0));
      const useExpandedLabels = (canvas.parentElement?.clientWidth || canvas.clientWidth || 0) >= 340;

      app.radarChart = new Chart(canvas, {
        type: 'radar',
        data: {
          labels,
          datasets: [{
            ...
            data: scores,
            fill: true,
            backgroundColor: 'rgba(108, 141, 113, 0.16)',
            borderColor: '#4d6a53',
            borderWidth: 2,
            pointBackgroundColor: '#4d6a53',
        ...
        options: {
          ...
          plugins: {
            ...
            tooltip: {
              callbacks: {
                ...
                label(context) {
                  return `得分 ${context.formattedValue} / 6`;
                }
              }
            }
          },
          scales: {
            r: {
              ...
              grid: {
                color: 'rgba(106, 120, 111, 0.18)'
              },
              angleLines: {
                color: 'rgba(106, 120, 111, 0.15)'
              },
              pointLabels: {
                color: '#4d6a53',
                callback(value, index) {
                  return useExpandedLabels ? expandedLabels[index] : value;
                },
                font: {
                  size: useExpandedLabels ? 10 : 11,
                  weight: '600'
                }
              }
            }
    ...
    function applyResultToScreen(result, options = {}) {
      const { imageSrc = '', persist = false } = options;
      ...
      document.getElementById('resultQuote').textContent = type.intro;
      ...
      const resolvedImageSrc = imageSrc || TYPE_IMAGES[type.code] || '';
      if (resolvedImageSrc) {
        posterImage.src = resolvedImageSrc;
      ...
      if (persist) saveResultToStorage(result, resolvedImageSrc);
      showScreen('result');
      window.requestAnimationFrame(() => renderRadarChart(result));
    }

    function renderResult() {
      const result = computeResult();
      applyResultToScreen(result, {
        imageSrc: TYPE_IMAGES[result.finalType.code],
        persist: true
      });
    }
    ...
    function restoreSavedResult() {
      ...
      applyResultToScreen(app.savedResult.result, {
        imageSrc: app.savedResult.imageSrc,
        persist: false
      });
      showToast('已恢复上次结果');
    }
    ...
    async function exportResultImage() {
      ...
      try {
        ...
        const [posterAsset, qrAsset] = await Promise.all([
          loadImage(document.getElementById('posterImage').getAttribute('src')),
          (async () => {
            try {
              return await loadImage(await ensureQrDataUrl());
            } catch (error) {
              return await loadImage(getQrUrl(320, 'png'));
            }
          })()
        ]);

        const gradient = ctx.createLinearGradient(0, 0, canvas.width, canvas.height);
        gradient.addColorStop(0, '#f8fdf8');
        gradient.addColorStop(0.5, '#eff7f0');
        gradient.addColorStop(1, '#e6f0e7');
        ctx.fillStyle = gradient;
        ctx.fillRect(0, 0, canvas.width, canvas.height);

        ctx.fillStyle = 'rgba(127, 165, 134, 0.10)';
        ctx.beginPath();
        ctx.arc(980, 90, 180, 0, Math.PI * 2);
        ctx.fill();
        ctx.beginPath();
        ctx.arc(100, 1220, 170, 0, Math.PI * 2);
        ctx.fill();

        const cardX = 54;
        const cardY = 72;
        const cardW = canvas.width - 108;
        const cardH = canvas.height - 144;

        fillRoundedRect(ctx, cardX, cardY, cardW, cardH, 34, 'rgba(255,255,255,0.94)');
        strokeRoundedRect(ctx, cardX, cardY, cardW, cardH, 34, '#dbe8dd');

        ctx.fillStyle = '#4d6a53';
        ctx.font = '700 36px "PingFang SC", "Microsoft YaHei", sans-serif';
        ctx.fillText('SBTI 人格测试结果', cardX + 34, cardY + 50);
        ctx.fillStyle = '#6a786f';
        ctx.font = '400 24px "PingFang SC", "Microsoft YaHei", sans-serif';
        ctx.fillText('免费在线测试你的隐藏人格类型', cardX + 34, cardY + 90);

        const heroX = cardX + 34;
        const heroY = cardY + 128;
        const heroW = cardW - 68;
        const posterX = heroX + 32;
        const posterY = heroY + 36;
        const posterW = 340;
        const posterH = 388;
        const infoX = posterX + posterW + 38;
        const infoW = heroX + heroW - infoX - 32;

        const titleFont = '700 56px "PingFang SC", "Microsoft YaHei", sans-serif';
        const titleLineHeight = 60;
        const titleY = posterY + 58;
        const titleMetrics = measureTextBlock(ctx, {
          text: `${type.code}（${type.cn}）`,
          maxWidth: infoW,
        ...
        const badgeY = titleY + titleMetrics.height + 18;
        const subFont = '400 22px "PingFang SC", "Microsoft YaHei", sans-serif';
        const subLineHeight = 30;
        const subY = badgeY + 74;
        const subMetrics = measureTextBlock(ctx, {
          text: result.sub,
          maxWidth: infoW,
        ...
        const quoteFont = '600 19px "PingFang SC", "Microsoft YaHei", sans-serif';
        const quoteLineHeight = 24;
        const quoteY = Math.max(subY + subMetrics.height + 18, posterY + 264);
        const quoteMetrics = measureTextBlock(ctx, {
          text: type.intro,
          maxWidth: infoW - 40,
        ...
        const quoteH = Math.max(88, 48 + quoteMetrics.height + 20);
        const heroH = Math.max(460, Math.max(posterY + posterH, quoteY + quoteH) - heroY + 36);

        fillRoundedRect(ctx, heroX, heroY, heroW, heroH, 28, '#fbfefb');
        strokeRoundedRect(ctx, heroX, heroY, heroW, heroH, 28, '#dbe8dd');

        fillRoundedRect(ctx, posterX, posterY, posterW, posterH, 24, '#f3f8f4');

        if (posterAsset) {
          ctx.save();
          roundRectPath(ctx, posterX, posterY, posterW, posterH, 24);
          ctx.clip();
          const ratio = Math.max(posterW / posterAsset.width, posterH / posterAsset.height);
          const drawW = posterAsset.width * ratio;
          const drawH = posterAsset.height * ratio;
          const drawX = posterX + (posterW - drawW) / 2;
          const drawY = posterY + (posterH - drawH) / 2;
          ctx.drawImage(posterAsset, drawX, drawY, drawW, drawH);
          ctx.restore();
        } else {
          ctx.fillStyle = '#4d6a53';
          ctx.font = '700 44px "PingFang SC", "Microsoft YaHei", sans-serif';
          ctx.textAlign = 'center';
          ctx.fillText(type.code, posterX + posterW / 2, posterY + 146);
          ctx.font = '400 24px "PingFang SC", "Microsoft YaHei", sans-serif';
          ctx.fillText(type.cn, posterX + posterW / 2, posterY + 206);
          ctx.textAlign = 'left';
        }

        ctx.fillStyle = '#4d6a53';
        ctx.font = '700 22px "PingFang SC", "Microsoft YaHei", sans-serif';
        ctx.fillText('我测出的主类型', infoX, posterY + 16);
        ...
        fillRoundedRect(ctx, infoX, badgeY, infoW, 56, 28, '#edf6ef');
        strokeRoundedRect(ctx, infoX, badgeY, infoW, 56, 28, '#dbe8dd');
        ctx.fillStyle = '#4d6a53';
        ctx.font = '700 20px "PingFang SC", "Microsoft YaHei", sans-serif';
        ctx.fillText(result.badge, infoX + 24, badgeY + 36);
        ...
        fillRoundedRect(ctx, infoX, quoteY, infoW, quoteH, 24, '#f3f8f4');
        strokeRoundedRect(ctx, infoX, quoteY, infoW, quoteH, 24, '#dbe8dd');
        ctx.fillStyle = '#4d6a53';
        ctx.font = '700 18px "PingFang SC", "Microsoft YaHei", sans-serif';
        ctx.fillText('人格小句', infoX + 20, quoteY + 24);
        drawTextBlock(ctx, {
          text: type.intro,
          x: infoX + 20,
          y: quoteY + 44,
        ...
        const summaryX = heroX;
        const summaryY = heroY + heroH + 34;
        const summaryW = heroW;
        const summaryTitleOffset = 54;
        const summaryBodyOffset = 84;
        const summaryLineHeight = 40;
        const summaryFont = '400 24px "PingFang SC", "Microsoft YaHei", sans-serif';
        const maxSummaryH = cardY + cardH - 34 - 140 - 34 - summaryY;
        const maxSummaryLines = Math.max(4, Math.floor((maxSummaryH - 120) / summaryLineHeight));
        const summaryMetrics = measureTextBlock(ctx, {
          text: type.desc,
          maxWidth: summaryW - 72,
          lineHeight: summaryLineHeight,
          font: summaryFont,
          maxLines: Math.min(7, maxSummaryLines)
        });
        const summaryH = Math.max(320, 120 + summaryMetrics.height);
        fillRoundedRect(ctx, summaryX, summaryY, summaryW, summaryH, 28, '#fbfefb');
        strokeRoundedRect(ctx, summaryX, summaryY, summaryW, summaryH, 28, '#dbe8dd');
        ctx.fillStyle = '#1e2a22';
        ctx.font = '700 32px "PingFang SC", "Microsoft YaHei", sans-serif';
        ctx.fillText('一句话解读', summaryX + 36, summaryY + summaryTitleOffset);
        drawTextBlock(ctx, {
          text: type.desc,
          x: summaryX + 36,
          y: summaryY + summaryBodyOffset,
          maxWidth: summaryW - 72,
          lineHeight: summaryLineHeight,
          color: '#304034',
          font: summaryFont,
        ...
        const shareX = heroX;
        const shareY = summaryY + summaryH + 34;
        const shareW = heroW;
        const shareH = 140;
        fillRoundedRect(ctx, shareX, shareY, shareW, shareH, 28, 'rgba(255,255,255,0.96)');
        strokeRoundedRect(ctx, shareX, shareY, shareW, shareH, 28, '#dbe8dd');
        ctx.fillStyle = '#1e2a22';
        ctx.font = '700 32px "PingFang SC", "Microsoft YaHei", sans-serif';
        ctx.fillText('扫码进入测试', shareX + 36, shareY + 54);
        
        ctx.fillStyle = '#6a786f';
        ctx.font = '400 24px "PingFang SC", "Microsoft YaHei", sans-serif';
        ctx.fillText(SITE_HOST, shareX + 36, shareY + 95);

        if (qrAsset) {
          const qrBoxY = shareY + 20;
          const qrBoxSize = 100;
          const qrBoxX = shareX + shareW - qrBoxSize - 20;
          ctx.drawImage(qrAsset, qrBoxX, qrBoxY, qrBoxSize, qrBoxSize);
        }

        const link = document.createElement('a');
        link.href = canvas.toDataURL('image/png');
        link.download = `sbti-${type.code.toLowerCase()}-result.png`;
    ...
    function renderAllTypesGrid() {
      const container = document.getElementById('allTypesGrid');
      if (!container) return;
      container.innerHTML = '';

      const title = document.getElementById('allTypesTitle');
      const subtitle = document.getElementById('allTypesSubtitle');
      const totalTypes = Object.keys(TYPE_LIBRARY).length;
      if (title) title.textContent = `共有 ${totalTypes} 种人格`;
      if (subtitle) {
        subtitle.textContent = `其中 ${NORMAL_TYPES.length} 种常规人格，${SPECIAL_TYPE_CODES.length} 种特殊触发人格。`;
      }
      ...
      ALL_TYPE_CODES.forEach(code => {
        ...
        let imgHtml = '';
        if (imgSrc) {
          imgHtml = `<img src="${imgSrc}" class="type-img-mini" alt="${code}" loading="lazy" decoding="async">`;
        }
        const isSpecial = SPECIAL_TYPE_CODES.includes(code);
        const specialTag = isSpecial ? '<div class="tc-tag-mini">特殊</div>' : '';
        
        card.innerHTML = `
          ${imgHtml}
          <div class="tc-code-mini">${code}</div>
          <div class="tc-cn-mini">${data.cn}</div>
          ${specialTag}
        `;
    ...
    // Make sure we also have it exported on load

    app.savedResult = loadSavedResultFromStorage();
    renderSavedResultCta();
    renderShareQr();
    ...
    document.getElementById('shareIntroBtn').addEventListener('click', shareSite);
    document.getElementById('backIntroBtn').addEventListener('click', () => showScreen('intro'));
    document.getElementById('shareTestBtn').addEventListener('click', shareSite);
    ...
    document.getElementById('copyResultBtn').addEventListener('click', copySiteLink);
    document.getElementById('exportBtn').addEventListener('click', exportResultImage);
